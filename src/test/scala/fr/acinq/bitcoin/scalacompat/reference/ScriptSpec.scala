...
object ScriptSpec {
  ...
  def creditTx(scriptPubKey: ByteVector, amount: Btc) = Transaction(version = 1,
    txIn = TxIn(OutPoint(TxHash(ByteVector32.Zeroes), -1), Script.write(OP_0 :: OP_0 :: Nil), 0xffffffff) :: Nil,
    txOut = TxOut(amount, scriptPubKey) :: Nil,
    lockTime = 0)

  def spendingTx(scriptSig: ByteVector, tx: Transaction) = Transaction(version = 1,
    txIn = TxIn(OutPoint(TxHash(Crypto.hash256(Transaction.write(tx))), 0), scriptSig, 0xffffffff) :: Nil,
