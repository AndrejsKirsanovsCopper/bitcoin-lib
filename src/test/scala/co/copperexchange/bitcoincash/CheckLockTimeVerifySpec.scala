...
class CheckLockTimeVerifySpec extends FlatSpec {
  "Bip65" should "let you initiate payment channels" in {
    ...
    val previousTx = Transaction.read(
      ""
    )
    ...
    val keyAlice = PrivateKey(
      hex"C0B91A94A26DC9BE07374C2280E43B1DE54BE568B2509EF3CE1ADE5C9CF9E8AA"
    )
    val pubAlice = keyAlice.publicKey

    val keyBob = PrivateKey(
      hex"5C3D081615591ABCE914D231BA009D8AE0174759E4A9AE821D97E28F122E2F8C"
    )
    ...
    val scriptPubKey = OP_IF ::
      OP_PUSHDATA(
        ByteVector(100: Byte)
      ) :: OP_CHECKLOCKTIMEVERIFY :: OP_DROP :: OP_PUSHDATA(
        pubAlice.value
      ) :: OP_CHECKSIG ::
      OP_ELSE ::
      OP_2 :: OP_PUSHDATA(pubAlice.value) :: OP_PUSHDATA(
        pubBob.value
      ) :: OP_2 :: OP_CHECKMULTISIG :: OP_ENDIF :: Nil
    ...
    val tx = {
      ...
      tmpTx.updateSigScript(
        0,
        OP_PUSHDATA(sig) :: OP_PUSHDATA(key.publicKey) :: Nil
      )
    }
    ...
    Transaction.correctlySpends(
      tx1,
      Seq(tx),
      ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS | ScriptFlags.SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY
    )
    ...
    val tx2 = {
      ...
      val sigScript =
        OP_0 :: OP_PUSHDATA(sig1) :: OP_PUSHDATA(sig2) :: OP_0 :: Nil
