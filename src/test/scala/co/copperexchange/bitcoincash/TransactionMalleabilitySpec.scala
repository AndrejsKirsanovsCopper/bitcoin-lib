...
class TransactionMalleabilitySpec extends FlatSpec {
  "Transaction" should "not be malleable" in {
    ...
    val prevTx = Transaction.read(
      ""
    )

    // private key that matches the public key the btc we sent to in the tx we want to redeem
    val privateKey = PrivateKey
      .fromBase58(
        "cSjAjBx5zSuA16zhG2owyNCzkXLc7qJTz9M8aR6uK9S9QqS9P6gF",
    ...
    val publicKey =
      hex"03f1c059112166776c70367cc1a83851c6224e45f6fe3944442f7961072212f954"
    val publicKeyScript = OP_DUP :: OP_HASH160 :: OP_PUSHDATA(
    ...
    val unsignedTx = Transaction(
      ...
      txOut = List(
        TxOut(amount = 1000 sat, publicKeyScript = publicKeyScript)
      ),
      lockTime = 0
    )
    ...
    val sig = Transaction.signInput(
      unsignedTx,
      0,
      prevTx.txOut(1).publicKeyScript,
      SIGHASH_ALL | SIGHASH_FORKID,
      prevTx.txOut(1).amount,
      privateKey
    )
    val signatureScript =
    ...
    Transaction.correctlySpends(
      tx2,
      Seq(prevTx),
      ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS & ~ScriptFlags.SCRIPT_VERIFY_SIGPUSHONLY
    )
