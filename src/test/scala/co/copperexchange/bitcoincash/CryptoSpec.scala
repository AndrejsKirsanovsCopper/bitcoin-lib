...
class CryptoSpec extends FlatSpec {
  ...
  it should "sign and verify signatures" in {
    ...
    assert(Crypto.verifySignature(data, compact2der(sig), publicKey, 0))
  }
  ...
  it should "recover public keys from signatures (basic test)" in {
    ...
    assert(verifySignature(message, compact2der(sig64), pub1, 0))
    assert(verifySignature(message, compact2der(sig64), pub2, 0))
  ...
  it should "recover public keys from signatures (random tests)" in {
    ...
    for (i <- 0 until 100) {
      ...
      assert(verifySignature(bytesMessage, compact2der(sig64), pub1, 0))
      assert(verifySignature(bytesMessage, compact2der(sig64), pub2, 0))
      assert(pub == pub1 || pub == pub2)
    }
  }
}
