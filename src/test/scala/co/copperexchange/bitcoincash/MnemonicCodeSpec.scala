...
class MnemonicCodeSpec extends FunSuite {
  ...
  test("reference tests") {
    ...
    val stream =
      classOf[MnemonicCodeSpec].getResourceAsStream("/bip39_vectors.json")
    val vectors =
      JsonMethods.parse(new InputStreamReader(stream)).extract[TestVectors]
    vectors.english.map(_ match {
      case Array(raw, mnemonics, seed, xprv) =>
        ...
        assert(
          toSeed(toMnemonics(bin), "TREZOR") === ByteVector.fromValidHex(seed)
        )
        val master = DeterministicWallet.generate(ByteVector.fromValidHex(seed))
        assert(
          DeterministicWallet.encode(master, DeterministicWallet.xprv) == xprv
        )
    })
  }
