  ...
  test("SIGHASH_ANYONECANPAY lets you add inputs") {
    ...
    val previousTx = Seq(
      Transaction(
        version = 2,
        txIn = Nil,
        txOut = TxOut(42 millibtc, Script.pay2pkh(publicKeys(0))) :: Nil,
        lockTime = 0
      ),
      Transaction(
        version = 2,
        txIn = Nil,
        txOut = TxOut(42 millibtc, Script.pay2pkh(publicKeys(1))) :: Nil,
        lockTime = 0
      )
    )
    ...
    val tx = Transaction(
      ...
      txOut = TxOut(
        80 millibtc,
        Script.pay2sh(Script.createMultiSigMofN(2, publicKeys))
      ) :: Nil,
      lockTime = 0L
    )
    ...
    val tx1 = {
      val tmp = tx.addInput(
        TxIn(
          OutPoint(previousTx(0), 0),
          sequence = 0xffffffffL,
          signatureScript = Nil
        )
      )
      val sig: ByteVector = Transaction.signInput(
        tmp,
        0,
        Script.pay2pkh(publicKeys(0)),
      ...
      tmp.updateSigScript(
        0,
        OP_PUSHDATA(sig) :: OP_PUSHDATA(publicKeys(0).value) :: Nil
      )
    ...
    val tx2 = {
      val tmp = tx1.addInput(
        TxIn(
          OutPoint(previousTx(1), 0),
          sequence = 0xffffffffL,
          signatureScript = Nil
        )
      )
      ...
      tmp.updateSigScript(
        1,
        OP_PUSHDATA(sig) :: OP_PUSHDATA(publicKeys(1).value) :: Nil
      )
    }
    Transaction.correctlySpends(
      tx2,
    ...
    val tx3 = tx2.copy(txOut =
      tx2.txOut.updated(0, tx2.txOut(0).copy(amount = 40 millibtc))
    )
