...
class CashAddrSpec extends FunSuite {
  ...
  test("legacy address translation") {
    ...
    inputs.map { case (legacy, address) =>
      val (addrPrefix, bin1) = Base58Check.decode(legacy)
      val addressComputed = CashAddr.encodeAddress(
        "bitcoincash",
        if (addrPrefix == Base58.Prefix.PubkeyAddress) CashAddr.Type.PubKey
        else CashAddr.Type.Script,
        bin1
      )
      val (_, addrType, bin2) = CashAddr.decodeAddress(address)
      val legacyComputed = Base58Check.encode(
        if (addrType == CashAddr.Type.PubKey) Base58.Prefix.PubkeyAddress
        else Base58.Prefix.ScriptAddress,
        bin1
      )
      assert(bin1 == bin2)
  ...
  test("auto-detect prefixes") {
    ...
    inputs.map { case (address, prefix) =>
  ...
  test("test vectors") {
    ...
    val cases = Seq(
      ...
      (
        "bitcoincash",
        CashAddr.Type.PubKey,
        hex"7ADBF6C17084BC86C1706827B41A56F5CA32865925E946EA",
      ...
      (
        "bitcoincash",
        CashAddr.Type.PubKey,
        hex"3A84F9CF51AAE98A3BB3A78BF16A6183790B18719126325BFC0C075B",
      ...
      (
        "bitcoincash",
        CashAddr.Type.PubKey,
        hex"3173EF6623C6B48FFD1A3DCC0CC6489B0A07BB47A37F47CFEF4FE69DE825C060",
        "bitcoincash:" +
      ...
      (
        "bitcoincash",
        CashAddr.Type.PubKey,
        hex"C07138323E00FA4FC122D3B85B9628EA810B3F381706385E289B0B25631197D194B5C238BEB136FB",
        "bitcoincash:" +
      ...
      (
        "bitcoincash",
        CashAddr.Type.PubKey,
        hex"E361CA9A7F99107C17A622E047E3745D3E19CF804ED63C5C40C6BA763696B98241223D8CE62AD48D863F4CB18C930E4C",
        "bitcoincash:" +
      ...
      (
        "bitcoincash",
        CashAddr.Type.PubKey,
        hex"D9FA7C4C6EF56DC4FF423BAAE6D495DBFF663D034A72D1DC7D52CBFE7D1E6858F9D523AC0A7A5C34077638E4DD1A701BD017842789982041",
        "bitcoincash:" +
    ...
    cases.map { case (prefix, version, content, expected) =>
