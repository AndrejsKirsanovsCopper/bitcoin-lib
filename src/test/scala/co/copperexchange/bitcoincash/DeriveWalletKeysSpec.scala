...
class DeriveWalletKeysSpec extends FunSuite {
  ...
  val mnemonics =
    "gun please vital unable phone catalog explain raise erosion zoo truly exist"
  val seed = MnemonicCode.toSeed(mnemonics, "")
  val master = DeterministicWallet.generate(seed)

  test("restore BIP44 wallet") {
    val account = DeterministicWallet.derivePrivateKey(
      master,
      DeterministicWallet.KeyPath("m/44'/1'/0'")
    )
    // some wallets will use tpub instead of upub
    val xpub = DeterministicWallet.encode(
      DeterministicWallet.publicKey(account),
      DeterministicWallet.tpub
    )
    assert(
      xpub == "tpubDDamug2qVwe94yFJ38MM3ek2LiWiyjMmkQPhYMnHNZz5XHj7bj8xc7pFmyiYnCfqrSy62e1196qcpmKYhcUMcBTGMW4mEWf1v9H8wNtLZku"
    )
    assert(
      deriveAddresses(xpub) == Seq(
        "bchtest:qpz3f2m6tpkzp7mc3ueamxnzkh7q6wh9ect8fmxs7z",
        "bchtest:qz8tej8qqm49mck4glprjx94n9e5la2s5ynt67ajgf",
        "bchtest:qz4rlzr6p0u05m3g3xyws29wwqtfchpg4qj3paqmgw",
        "bchtest:qp9gs0ph6gqdg246tths4vfvfndzngekvc6qq38nwg",
        "bchtest:qpj76pa2n03xw48fxfqckycwr6uxra4vavxywhff8x"
      )
    )
  }
}
    ...
    for (i <- 0L until 5L) yield {
      ...
      val address = prefix match {
        case DeterministicWallet.tpub =>
          computeP2PkhAddress(pub.publicKey, Block.TestnetGenesisBlock.hash)
        case DeterministicWallet.xpub =>
          computeP2PkhAddress(pub.publicKey, Block.LivenetGenesisBlock.hash)
      }
      address
    }
  }
