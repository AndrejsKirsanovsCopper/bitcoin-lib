...
class Secp256k1Spec extends FunSuite {
  ...
  test("deterministic signatures") {
    ...
    for (i <- 0 until 1000) {
      ...
      val sig1: ByteVector =
        Crypto.sign(ByteVector.view(data), PrivateKey(ByteVector.view(priv)))
      val sig2: ByteVector =
        ByteVector.view(nativeSecp256k1.get.sign(data, priv))
      assert(sig1 == sig2)
    }
  }
  test("ecdh") {
    ...
    for (i <- 0 until 1000) {
      ...
      val secret1: ByteVector = Crypto.ecdh(
        PrivateKey(ByteVector.view(priv1)),
        PrivateKey(ByteVector.view(priv2)).publicKey
      )
      val secret2: ByteVector = ByteVector.view(
        nativeSecp256k1.get.ecdh(priv1, Secp256k1.get().pubkeyCreate(priv2))
      )
