...
class ScriptSpec extends FlatSpec {
  "Script" should "parse signature scripts" in {
    ...
    assert(
      Base58Check.encode(
        ...
        hash
      ) === "mkFQohBpy2HDXrCwyMrYL5RtfrmeiuuPY2"
    )
  }
  it should "parse 'pay to public key' scripts" in {
    val blob = BaseEncoding
      .base16()
      .lowerCase()
      .decode("76a91433e81a941e64cda12c6a299ed322ddbdd03f8d0e88ac")
    val script = Script.parse(blob)
  ...
  it should "parse 'pay to script' scripts" in {
    val blob = BaseEncoding
      .base16()
      .lowerCase()
      .decode("a914a90003b4ddef4be46fc61e7f2167da9d234944e287")
    val script = Script.parse(blob)
    val OP_HASH160 :: OP_PUSHDATA(scriptHash, _) :: OP_EQUAL :: Nil = script
    val multisigAddress =
      Base58Check.encode(Prefix.ScriptAddressTestnet, scriptHash)
  ...
  it should "parse if/else/endif" in {
    val tx = Transaction(
      version = 1,
      txIn = TxIn(
        OutPoint(ByteVector32.Zeroes, 0xffffffff),
        Script.write(OP_NOP :: Nil),
        0xffffffff
      ) :: Nil,
    ...
    val script =
      OP_1 :: OP_2 :: OP_EQUAL :: OP_IF :: OP_3 :: OP_ELSE :: OP_4 :: OP_ENDIF :: Nil
    val stack = runner.run(script)
    assert(stack === List(ByteVector(4)))
    val script1 =
      OP_1 :: OP_1 :: OP_EQUAL :: OP_IF :: OP_3 :: OP_ELSE :: OP_4 :: OP_ENDIF :: Nil
    val stack1 = runner.run(script1)
    assert(stack1 === List(ByteVector(3)))
    val script2 =
      OP_1 :: OP_1 :: OP_EQUAL :: OP_IF :: OP_3 :: OP_3 :: OP_EQUAL :: OP_IF :: OP_5 :: OP_ENDIF :: OP_ELSE :: OP_4 :: OP_ENDIF :: Nil
  ...
  it should "encode/decode simple numbers" in {
    for (i <- -1 to 16) {
      assert(
        Script.decodeNumber(
          Script.encodeNumber(i),
          checkMinimalEncoding = true
        ) === i
      )
    }
  }
