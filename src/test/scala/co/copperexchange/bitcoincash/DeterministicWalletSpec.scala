...
class DeterministicWalletSpec extends FlatSpec {
  ...
  "Deterministic Wallet" should "generate and derive keys (test vector #1)" in {
    ...
    assert(
      encode(
        derivePrivateKey(
          m,
          hardened(0) :: 1L :: hardened(2) :: 2L :: 1000000000L :: Nil
        ),
        xprv
      ) === "xprvA41z7zogVVwxVSgdKUHDy1SKmdb533PjDz7J6N6mV6uS3ze1ai8FHa8kmHScGpWmj4WggLyQjgPie1rFSruoUihUZREPSL39UNdE3BBDu76"
    )
  ...
  it should "generate and derive keys (test vector #2)" in {
    ...
    val m0_2147483647h_1_2147483646h =
      derivePrivateKey(m0_2147483647h_1, hardened(2147483646))
    assert(
      encode(
    ...
    val m0_2147483647h_1_2147483646h_pub =
      publicKey(m0_2147483647h_1_2147483646h)
    assert(
      encode(
    ...
    val m0_2147483647h_1_2147483646h_2 =
      derivePrivateKey(m0_2147483647h_1_2147483646h, 2)
    assert(
      m0_2147483647h_1_2147483646h_2.path.toString === "m/0/2147483647'/1/2147483646'/2"
    )
    assert(
      encode(
    ...
    val m0_2147483647h_1_2147483646h_2_pub =
      publicKey(m0_2147483647h_1_2147483646h_2)
    assert(
      encode(
  ...
  it should "generate and derive keys (test vector #3)" in {
    ...
    assert(
      encode(
        derivePrivateKey(m, hardened(0)),
        xprv
      ) === "xprv9uPDJpEQgRQfDcW7BkF7eTya6RPxXeJCqCJGHuCJ4GiRVLzkTXBAJMu2qaMWPrS7AANYqdq6vcBcBUdJCVVFceUvJFjaPdGZ2y9WACViL4L"
    )
    assert(
      encode(
        publicKey(derivePrivateKey(m, hardened(0))),
        xpub
  ...
  it should "be possible to go up the private key chain if you have the master pub key and a child private key!!" in {
    ...
    val k =
      new BigInteger(1, m.secretkeybytes.toArray) // k is our master private key
    ...
    val I = Crypto.hmac512(
      m_pub.chaincode,
      m_pub.publickeybytes ++ writeUInt32(42, ByteOrder.BIG_ENDIAN)
    )
    val IL = I.take(32)
    val guess = new BigInteger(1, m42.secretkeybytes.toArray)
      .subtract(new BigInteger(1, IL.toArray))
      .mod(Crypto.curve.getN)
    assert(guess === k)
  }

  it should "parse string-formatted derivation paths" in {
    assert(
      KeyPath("m/44'/0'/0'/0") == KeyPath(
        hardened(44) :: hardened(0) :: hardened(0) :: 0L :: Nil
      )
    )
    assert(
      KeyPath("/44'/0'/0'/0") == KeyPath(
        hardened(44) :: hardened(0) :: hardened(0) :: 0L :: Nil
      )
    )
    assert(
    ...
    assert(
      KeyPath("m/44/0'/0'/0") == KeyPath(
        44L :: hardened(0) :: hardened(0) :: 0L :: Nil
      )
    )
  ...
  it should "be able to derive private keys" in {
    ...
    for (_ <- 0 until 50) {
      ...
      for (_ <- 0 until 50) {
        ...
        val (prefix, decoded) =
          DeterministicWallet.ExtendedPrivateKey.decode(encoded)
        assert(prefix == DeterministicWallet.tprv)
        assert(
          decoded.chaincode == priv.chaincode && decoded.secretkeybytes == priv.secretkeybytes
        )

        val pub = DeterministicWallet.publicKey(priv)
        val encoded1 = DeterministicWallet.encode(pub, DeterministicWallet.tpub)
        val (prefix1, decoded1) =
          DeterministicWallet.ExtendedPublicKey.decode(encoded1)
        assert(prefix1 == DeterministicWallet.tpub)
        assert(
          decoded1.chaincode == pub.chaincode && decoded1.publicKey == pub.publicKey
        )
      }
    }
  }
