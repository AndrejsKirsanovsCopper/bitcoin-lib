...
class ToolsSpec extends FlatSpec {
  ...
  it should "read/write uint32" in {
    Seq(0, 15, 1550, 0xa0f1, 0xfef1, 0xffff, 0x00000000, 0x00000001,
      0x10000000L, 0xaef5ff86L, 0xfeffaf05L, 0xffffffL).map(value => {
      val out = new ByteArrayOutputStream()
  ...
  it should "read/write uint64" in {
    Seq(0, 15, 1550, 0xa0f1, 0xfef1, 0xffff, 0x00000000, 0x00000001,
      0x10000000L, 0xaef5ff86L, 0xfeffaf05L, 0xffffffL, 0x1000000000000000L,
      0xffffffffffffffffL).map(value => {
      val out = new ByteArrayOutputStream()
