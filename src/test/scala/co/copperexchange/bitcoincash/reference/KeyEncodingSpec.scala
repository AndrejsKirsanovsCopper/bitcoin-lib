...
class KeyEncodingSpec extends FunSuite {
  ...
  test("valid keys") {
    val stream =
      classOf[KeyEncodingSpec].getResourceAsStream("/data/key_io_valid.json")
  ...
  test("invalid keys") {
    val stream =
      classOf[KeyEncodingSpec].getResourceAsStream("/data/key_io_invalid.json")
    val json = JsonMethods.parse(new InputStreamReader(stream))
      ...
      _ match {
        ...
        case unexpected =>
          throw new IllegalArgumentException(
            s"don't know how to parse $unexpected"
          )
      }
    }
  }
...
object KeyEncodingSpec {
  def isValidBase58(input: String): Boolean = Try {
    ...
    prefix match {
      case Base58.Prefix.SecretKey | Base58.Prefix.SecretKeyTestnet =>
        Try(PrivateKey.fromBin(bin)).isSuccess
      case Base58.Prefix.PubkeyAddress | Base58.Prefix.PubkeyAddressTestnet =>
        bin.length == 20
      case _ => false
    }
  } getOrElse (false)
    ...
    data match {
      case JString(encoded) :: JString(hex) :: obj :: Nil => {
        ...
        if (isPrivkey) {
          ...
          assert(
            (isCompressed && data.length == 33 && data.last == 0x01) || (!isCompressed && data.length == 32)
          )
          assert(
            (chain == "main" && version == Base58.Prefix.SecretKey) || (chain != "main" && version == Base58.Prefix.SecretKeyTestnet)
          )
          ...
          encoded.head match {
            case '1' | 'm' | 'n' =>
              val (version, data) = Base58Check.decode(encoded)
              assert(
                (chain == "main" && version == Base58.Prefix.PubkeyAddress) || (chain != "main" && version == Base58.Prefix.PubkeyAddressTestnet)
              )
              assert(
                Script.write(
            ...
            case '2' | '3' =>
              val (version, data) = Base58Check.decode(encoded)
              assert(
                (chain == "main" && version == Base58.Prefix.ScriptAddress) || (chain != "main" && version == Base58.Prefix.ScriptAddressTestnet)
              )
              assert(
                Script.write(
                  OP_HASH160 :: OP_PUSHDATA(data) :: OP_EQUAL :: Nil
                ) == bin
              )
          }
        }
