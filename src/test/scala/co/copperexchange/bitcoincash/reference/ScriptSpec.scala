package co.copperexchange.bitcoincash.reference

import java.io.InputStreamReader

import co.copperexchange.bitcoincash._
...
object ScriptSpec {
  def parseFromText(input: String): ByteVector = {
    def parseInternal(
        tokens: List[String],
        acc: ByteVector = ByteVector.empty
    ): ByteVector = tokens match {
      case Nil => acc
      case head :: tail if head.matches("^-?[0-9]*$") =>
        head.toLong match {
          case -1 =>
            parseInternal(tail, acc :+ ScriptElt.elt2code(OP_1NEGATE).toByte)
          case 0 => parseInternal(tail, acc :+ ScriptElt.elt2code(OP_0).toByte)
          case value if value >= 1 && value <= 16 =>
            val bytes = ByteVector(
              (ScriptElt.elt2code(OP_1) - 1 + value).toByte
            )
      ...
      case head :: tail if ScriptElt.name2code.get(head).isDefined =>
        parseInternal(tail, acc :+ ScriptElt.name2code(head).toByte)
      case head :: tail if head.startsWith("0x") =>
        parseInternal(tail, acc ++ ByteVector.fromValidHex(head))
      case head :: tail if head.startsWith("'") && head.endsWith("'") =>
        parseInternal(
          ...
          acc ++ Script.write(
            OP_PUSHDATA(
              ByteVector.view(
                head.stripPrefix("'").stripSuffix("'").getBytes("UTF-8")
              )
            ) :: Nil
          )
        )
    ...
    try {
      val tokens =
        input.split(' ').filterNot(_.isEmpty).map(_.stripPrefix("OP_")).toList
  ...
  def parseScriptFlags(strFlags: String): Int = if (strFlags.isEmpty) 0
  else strFlags.split(",").map(mapFlagNames(_)).foldLeft(0)(_ | _)

  def creditTx(scriptPubKey: ByteVector, amount: Btc) = Transaction(
    version = 1,
    txIn = TxIn(
      OutPoint(ByteVector32.Zeroes, -1),
      Script.write(OP_0 :: OP_0 :: Nil),
      TxIn.SEQUENCE_FINAL
    ) :: Nil,
  ...
  def spendingTx(scriptSig: ByteVector, tx: Transaction) = Transaction(
    version = 1,
    txIn = TxIn(
      OutPoint(Crypto.hash256(Transaction.write(tx)), 0),
      scriptSig,
      TxIn.SEQUENCE_FINAL
    ) :: Nil,
  ...
  def runTest(
    ...
    val result =
      Try(runner.verifyScripts(scriptSig, scriptPubKey)).getOrElse(false)
    val expected = expectedText == "OK"
    if (result != expected) {
      throw new RuntimeException(
        comments.getOrElse(
          s"ScriptSig: $scriptSigText - ScriptPubKey: $scriptPubKeyText - Flags: $flags - Expected: $expectedText"
        )
      )
    }
  }
  ...
  def runTest(json: JValue): Int = {
    ...
    json
      .extract[List[List[JValue]]]
      .tail
      .foreach(_ match {
        case JString(comment) :: Nil => ()
        case JString(scriptSig) :: JString(scriptPubKey) :: JString(
              flags
            ) :: JString(expected) :: JString(comments) :: Nil =>
          ScriptSpec
            .runTest(scriptSig, scriptPubKey, flags, Some(comments), expected)
          count = count + 1
        case JString(scriptSig) :: JString(scriptPubKey) :: JString(
              flags
            ) :: JString(expected) :: Nil =>
          ScriptSpec.runTest(scriptSig, scriptPubKey, flags, None, expected)
          count = count + 1
        case JArray(m) :: JString(scriptSig) :: JString(
              scriptPubKey
            ) :: JString(flags) :: JString(expected) :: JString(
              comments
            ) :: Nil =>
          val amount: Btc = m.last match {
            case JDouble(value) => Btc(value)
            case unexpected =>
              throw new RuntimeException(s"expected an amount, got $unexpected")
          }
          ScriptSpec.runTest(
            amount,
            scriptSig,
        ...
        case JArray(m) :: JString(scriptSig) :: JString(
              scriptPubKey
            ) :: JString(flags) :: JString(expected) :: Nil =>
          val amount: Btc = m.last match {
            case JDouble(value) => Btc(value)
            case unexpected =>
              throw new RuntimeException(s"expected an amount, got $unexpected")
          ...
          ScriptSpec
            .runTest(amount, scriptSig, scriptPubKey, flags, None, expected)
          count = count + 1
        case unexpected =>
          throw new RuntimeException(s"cannot parse $unexpected")
      })
    count
  }
}
...
class ScriptSpec extends FunSuite {
  test("reference client script tests") {
    val stream =
      classOf[ScriptSpec].getResourceAsStream("/data/script_tests.json")
