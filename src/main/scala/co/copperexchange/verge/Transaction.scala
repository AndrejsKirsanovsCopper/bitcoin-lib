...
import co.copperexchange.verge.Script.Runner
...
object Transaction extends BtcSerializer[Transaction] {
  ...
  override def read(input: InputStream, protocolVersion: Long): Transaction = {
    val tx = Transaction(uint32(input), uint32(input), readCollection[TxIn](input, protocolVersion), Seq.empty[TxOut], 0)
...
/**
  ...
  */
case class Transaction(version: Long, time: Long, txIn: Seq[TxIn], txOut: Seq[TxOut], lockTime: Long) extends BtcSerializable[Transaction] {
