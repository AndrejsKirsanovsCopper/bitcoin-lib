...
import co.copperexchange.bitcoincash.MerkleBlock.topHeight
...
case class MerkleBlock(
    ...
    nonce: Long,
    txCount: Int,
    hashes: Seq[ByteVector],
    flags: ByteVector
) extends BtcSerializable[MerkleBlock] {
  require(
    previousBlockHash.length == 32,
    "length of preivous block hash should be 32"
  )
...
object MerkleBlock extends BtcSerializer[MerkleBlock] {
  ...
  def isBitSet(byte: Byte, bit: Int): Boolean =
    ((byte.toInt >> bit) & 0x01) != 0

  def toBits(byte: Byte): List[Boolean] =
    (for (i <- 0 to 7) yield isBitSet(byte, i)).toList
  ...
  def topHeight(leafCount: Int): Int = {
    ...
    def loop(height: Int): Int =
      if (calcTreeWidth(leafCount, height) > 1) loop(height + 1) else height
  ...
  def computeRoot(
      ...
      hashes: Seq[ByteVector],
      bits: List[Boolean],
      matched: List[(ByteVector, Int)]
  ): (ByteVector, List[(ByteVector, Int)], Seq[ByteVector], List[Boolean]) = {
    bits match {
      case false :: tail => (hashes.head, matched, hashes.tail, tail)
      case true :: tail if height == 0 =>
        (hashes.head, (hashes.head, pos) :: matched, hashes.tail, tail)
      case true :: tail if (pos * 2 + 1) < calcTreeWidth(count, height - 1) =>
        ...
        val (right, matched2, hashes2, bits2) =
          computeRoot(count, height - 1, 2 * pos + 1, hashes1, bits1, matched1)
        require(left != right)
  ...
  def verify(merkleBlock: MerkleBlock): Unit = {
    val (root, matched, hashes, bits) = computeRoot(
      merkleBlock.txCount,
      topHeight(merkleBlock.txCount),
      0,
      merkleBlock.hashes,
      toBits(merkleBlock.flags),
      Nil
    )
