...
import co.copperexchange.bitcoincash.Crypto.{
  getHashType,
  hash160,
  verifySignature,
  PublicKey
}
import co.copperexchange.bitcoincash.ScriptElt.{code2elt, elt2code}
...
object ScriptFlags {
  ...
  val SCRIPT_ENABLE_SCHNORR_MULTISIG = (1 << 21)
  ...
  /** Mandatory script verification flags that all new blocks must comply with for
    ...
    * details.
    */
  val MANDATORY_SCRIPT_VERIFY_FLAGS =
...
object Script {
  ...
  def decodeBitField(input: ByteVector, size: Int): BitVector = {
    if (size > 32)
      throw new RuntimeException(
        "Schnorr multisig checkbits implementation assumes max 32 pubkeys"
      )
    if (input.size != (size + 7) / 8)
      throw new RuntimeException("invalid bitfield size")
    val res = input.bits.reverseByteOrder
    if (res.take(res.size - size).toInt() != 0)
      throw new RuntimeException("invalid bitfield bits")
    res.reverse.take(size)
  }
  ...
  def getSigOpCount(
    ...
    script
      .foldLeft[(Int, Option[ScriptElt])]((0, None)) {
        case ((n, prevOpcode), opcode) =>
          val inc = opcode match {
            ...
            case OP_CHECKDATASIG | OP_CHECKDATASIGVERIFY =>
              if ((scriptFlag & SCRIPT_VERIFY_CHECKDATASIG_SIGOPS) != 0) 1
              else 0
            case OP_CHECKMULTISIG | OP_CHECKMULTISIGVERIFY =>
              if (accurate) prevOpcode match {
                case Some(OP_1)  => 1
                case Some(OP_2)  => 2
                case Some(OP_3)  => 3
                case Some(OP_4)  => 4
  ...
  class Runner(
      context: Context,
      var scriptFlag: Int = MANDATORY_SCRIPT_VERIFY_FLAGS,
    ...
    def checkSignature(
      ...
      if (pubKey.isEmpty) false
      else if (sigBytes.isEmpty) false
      else if (!Crypto.isPubKeyValidLax(pubKey)) false
      else {
    ...
    def checkECDSASignatures(
        pubKeys: Seq[ByteVector],
        sigs: Seq[ByteVector],
        scriptCode: ByteVector
    ): Boolean =
      if (sigs.isEmpty) true
      else if (sigs.length > pubKeys.length) false
      else {
        val sig = sigs.head
        if (!Crypto.checkTransactionECDSASignatureEncoding(sig, scriptFlag))
          throw new RuntimeException("invalid signature")
        if (!Crypto.checkPubKeyEncoding(pubKeys.head, scriptFlag))
          throw new RuntimeException("invalid public key")
        if (checkSignature(pubKeys.head, sig, scriptCode))
          checkECDSASignatures(pubKeys.tail, sigs.tail, scriptCode)
        else
          checkECDSASignatures(pubKeys.tail, sigs, scriptCode)
      }

    def checkSchnorrSignatures(
        pubKeys: Seq[ByteVector],
        sigs: Seq[ByteVector],
        checkBits: BitVector,
        scriptCode: ByteVector
    ): Boolean = {
      val usedPubKeys = pubKeys.zipWithIndex
        .filter { case (_, idx) =>
          checkBits(idx)
        }
        .map(_._1)
      sigs.zip(usedPubKeys).forall { case (sig, pubKey) =>
        if (!Crypto.checkTransactionSchnorrSignatureEncoding(sig, scriptFlag))
          throw new RuntimeException("invalid signature")
        if (!Crypto.checkPubKeyEncoding(pubKey, scriptFlag))
          throw new RuntimeException("invalid public key")
        checkSignature(pubKey, sig, scriptCode)
      }
    }
    ...
    final def run(
      ...
      script match {
        ...
        case Nil => stack
        case op :: _ if isDisabled(op, scriptFlag) =>
        ...
        case OP_IF :: tail =>
          stack match {
            case True :: stacktail
                if (scriptFlag & SCRIPT_VERIFY_MINIMALIF) != 0 =>
              run(
                tail,
                stacktail,
                state.copy(
            ...
            case _ :: _ if (scriptFlag & SCRIPT_VERIFY_MINIMALIF) != 0 =>
            ...
            case _ => throw new IllegalArgumentException("unknown case")
          }
        ...
        case head :: tail if conditions.contains(false) =>
          run(tail, stack, state.copy(opCount = opCount + cost(head)))
        ...
        case op :: tail if isSimpleValue(op) =>
          run(tail, encodeNumber(simpleValue(op)) :: stack, state)
        case OP_NOP :: tail =>
        ...
        case op :: tail if isUpgradableNop(op) =>
          run(tail, stack, state.copy(opCount = opCount + 1))
        case OP_1ADD :: _ if stack.isEmpty =>
        ...
        case OP_1SUB :: _ if stack.isEmpty =>
        ...
        case OP_ABS :: _ if stack.isEmpty =>
          throw new RuntimeException("cannot run OP_ABS on am empty stack")
        case OP_ABS :: tail =>
          run(
            tail,
            encodeNumber(decodeNumber(stack.head).abs) :: stack.tail,
            state.copy(opCount = opCount + 1)
          )
        case OP_ADD :: tail =>
          stack match {
          ...
          stack match {
            case x1 :: x2 :: stacktail =>
              if (x1.length != x2.length)
                throw new RuntimeException("inputs must be the same size")
              val result = (x2 zipWith x1) { (ch1, ch2) => (ch1 & ch2).toByte }
              run(tail, result :: stacktail, state.copy(opCount = opCount + 1))
            case _ =>
              throw new RuntimeException(
                "cannot run OP_AND on a stack of less than 2 elements"
              )
          }
        case OP_BIN2NUM :: _ if stack.isEmpty =>
          throw new RuntimeException("cannot run OP_BIN2NUM on am empty stack")
        case OP_BIN2NUM :: tail =>
          val n = minimallyEncode(stack.head)
          require(
            isMinimallyEncoded(n),
        ...
        case OP_CAT :: tail =>
          stack match {
            case x1 :: x2 :: stacktail =>
              if (x1.length + x2.length > MaxScriptElementSize)
                throw new RuntimeException("concatenated string is too long")
              val result = x2 ++ x1
              run(tail, result :: stacktail, state.copy(opCount = opCount + 1))
            case _ =>
              throw new RuntimeException(
                "cannot run OP_CAT on a stack of less than 2 elements"
              )
          }
        ...
        case OP_CHECKSIG :: tail =>
          stack match {
            case pubKey :: sigBytes :: stacktail => {
              if (
                !Crypto.checkTransactionSignatureEncoding(sigBytes, scriptFlag)
              ) throw new RuntimeException("invalid signature")
              if (!Crypto.checkPubKeyEncoding(pubKey, scriptFlag))
                throw new RuntimeException("invalid public key")
              ...
              val scriptCode1 =
                if (
                  (scriptFlag & SCRIPT_ENABLE_SIGHASH_FORKID) == 0 || !isForkId(
                    getHashType(sigBytes)
                  )
                ) removeSignature(scriptCode, sigBytes)
                else scriptCode
              val success =
                checkSignature(pubKey, sigBytes, Script.write(scriptCode1))
        ...
        case OP_CHECKSIGVERIFY :: tail =>
          run(
            OP_CHECKSIG :: OP_VERIFY :: tail,
            stack,
            state.copy(opCount = opCount - 1)
          )
        case OP_CHECKDATASIG :: tail =>
          stack match {
            case pubKey :: message :: sig :: stacktail =>
              if (!Crypto.checkDataSignatureEncoding(sig, scriptFlag))
                throw new RuntimeException("invalid signature")
              if (!Crypto.checkPubKeyEncoding(pubKey, scriptFlag))
                throw new RuntimeException("invalid public key")
              val hash = Crypto.sha256(message)
              val success = verifySignature(
                hash,
                sig,
                PublicKey.fromBin(pubKey),
                scriptFlag
              ...
              if (
                !success && (scriptFlag & SCRIPT_VERIFY_NULLFAIL) != 0 && sig.nonEmpty
              )
                throw new RuntimeException(
                  "Signature must be zero for failed OP_CHECKDATASIG operation"
                )
              run(
                tail,
                (if (success) True else False) :: stacktail,
            ...
            case _ =>
              throw new RuntimeException(
                "Cannot perform OP_CHECKDATASIG on a stack with less than 3 elements"
              )
          }
        case OP_CHECKDATASIGVERIFY :: tail =>
          run(
            OP_CHECKDATASIG :: OP_VERIFY :: tail,
            stack,
            state.copy(opCount = opCount - 1)
          )
        case OP_CHECKMULTISIG :: tail => {
          ...
          val success =
            if (
              (scriptFlag & ScriptFlags.SCRIPT_ENABLE_SCHNORR_MULTISIG) != 0 && stack3(
                n
              ).nonEmpty
            ) {
              val checkBits = decodeBitField(stack3(n), m)
              require(
                checkBits.populationCount == n,
                "The bitfield doesn't set the right number of signatures"
              )
              checkSchnorrSignatures(
                pubKeys.reverse,
                sigs.reverse,
                checkBits,
              ...
              // Remove signature for pre-fork scripts
              val scriptCode1 = sigs.foldLeft(scriptCode)((scriptCode, sigBytes) =>
                if (
                  (scriptFlag & SCRIPT_ENABLE_SIGHASH_FORKID) == 0 || !isForkId(
                    getHashType(sigBytes)
                  )
              ...
              checkECDSASignatures(pubKeys, sigs, Script.write(scriptCode1))
            }
          if (!success && (scriptFlag & SCRIPT_VERIFY_NULLFAIL) != 0) {
            sigs.foreach(sig =>
              require(
        ...
        case OP_CHECKMULTISIGVERIFY :: tail =>
          run(
            OP_CHECKMULTISIG :: OP_VERIFY :: tail,
            stack,
            state.copy(opCount = opCount - 1)
          )
        case OP_CODESEPARATOR :: tail =>
          run(tail, stack, state.copy(opCount = opCount + 1, scriptCode = tail))
        case OP_DEPTH :: tail =>
          run(
            tail,
        ...
        case OP_DIV :: tail =>
          stack match {
            case x1 :: x2 :: stacktail =>
              ...
              if (bn2 == 0)
                throw new RuntimeException(
                  "cannot run OP_DIV with denominator equals to 0"
                )
              val result = bn1 / bn2
              run(
                tail,
            ...
            case _ =>
              throw new RuntimeException(
                "cannot run OP_DIV on a stack of less than 2 elements"
              )
          }
        case OP_SIZE :: _ if stack.isEmpty =>
        ...
        case OP_DROP :: tail =>
          run(tail, stack.tail, state.copy(opCount = opCount + 1))
        case OP_2DROP :: tail =>
          run(tail, stack.tail.tail, state.copy(opCount = opCount + 1))
        case OP_DUP :: tail =>
          run(tail, stack.head :: stack, state.copy(opCount = opCount + 1))
        case OP_2DUP :: tail =>
          stack match {
            case x1 :: x2 :: stacktail =>
              run(
        ...
        case OP_EQUAL :: tail =>
          stack match {
            case a :: b :: stacktail if a != b =>
              run(tail, False :: stacktail, state.copy(opCount = opCount + 1))
            case a :: b :: stacktail =>
              run(tail, True :: stacktail, state.copy(opCount = opCount + 1))
            case _ =>
              throw new RuntimeException(
        ...
        case OP_IFDUP :: tail =>
          stack match {
            ...
            case head :: _ if castToBoolean(head) =>
              run(tail, head :: stack, state.copy(opCount = opCount + 1))
        ...
        case OP_MOD :: tail =>
          stack match {
            case x1 :: x2 :: stacktail =>
              ...
              if (bn2 == 0)
                throw new RuntimeException(
                  "cannot run OP_MOD with divisor equals to 0"
                )
              val result = bn1 % bn2
              run(
                tail,
            ...
            case _ =>
              throw new RuntimeException(
                "cannot run OP_MOD on a stack of less than 2 elements"
              )
          }
        case OP_NEGATE :: _ if stack.isEmpty =>
        ...
        case OP_NIP :: tail =>
          stack match {
            case x1 :: x2 :: stacktail =>
              run(tail, x1 :: stacktail, state.copy(opCount = opCount + 1))
            case _ =>
              throw new RuntimeException(
                "Cannot perform OP_NIP on a stack with less than 2 elements"
              )
          }
        case OP_NOT :: _ if stack.isEmpty =>
        ...
        case OP_0NOTEQUAL :: _ if stack.isEmpty =>
          throw new RuntimeException(
        ...
        case OP_NUM2BIN :: tail =>
          stack match {
            case x1 :: x2 :: stacktail =>
              ...
              if (rawnum.size > size)
                throw new RuntimeException(
                  "unable to fit the number in the number of byte requested"
                )
              val result =
                if (rawnum.size == size) rawnum
                  ...
                  rawnum.take(rawnum.size - 1) ++ ByteVector(
                    (rawnum.last & 0x7f).toByte
                  ) ++ zeroes ++ ByteVector(signbit)
                }
              run(tail, result :: stacktail, state.copy(opCount = opCount + 1))
            case _ =>
              throw new RuntimeException(
                "cannot run OP_NUM2BIN on a stack of less than 2 elements"
              )
          }
        ...
        case OP_OR :: tail =>
          stack match {
            ...
            case _ =>
              throw new RuntimeException(
                "cannot run OP_OR on a stack of less than 2 elements"
              )
          }
        case OP_OVER :: tail =>
          stack match {
            case _ :: x2 :: _ =>
              run(tail, x2 :: stack, state.copy(opCount = opCount + 1))
            case _ =>
              throw new RuntimeException(
        ...
        case OP_2OVER :: tail =>
          stack match {
            case _ :: _ :: x3 :: x4 :: _ =>
              run(tail, x3 :: x4 :: stack, state.copy(opCount = opCount + 1))
            case _ =>
              throw new RuntimeException(
        ...
        case OP_SPLIT :: tail =>
          stack match {
            case x1 :: x2 :: stacktail =>
              ...
              if (position < 0 || position > x2.length)
                throw new RuntimeException("split point is not apropriate")
            ...
            case _ =>
              throw new RuntimeException(
                "cannot run OP_SPLIT on a stack of less than 2 elements"
              )
          }
        case OP_SUB :: tail =>
          stack match {
        ...
        case OP_TOALTSTACK :: tail =>
          run(tail, stack.tail, state.copy(altstack = stack.head :: altstack))
        case OP_TUCK :: tail =>
          stack match {
            case x1 :: x2 :: stacktail =>
              run(
        ...
        case OP_XOR :: tail =>
          stack match {
            ...
            case _ =>
              throw new RuntimeException(
                "cannot run OP_XOR on a stack of less than 2 elements"
              )
          }
          case _ => throw new IllegalArgumentException("unknown case")
      }
    ...
    def verifyScripts(
      ...
      if ((scriptFlag & SCRIPT_ENABLE_SIGHASH_FORKID) != 0)
        scriptFlag |= SCRIPT_VERIFY_STRICTENC
      ...
      val stack1 =
        if (
          ...
          if (
            (scriptFlag & SCRIPT_DISALLOW_SEGWIT_RECOVERY) == 0 && stack.size == 1 &&
  ...
  def createMultiSigMofN(m: Int, pubkeys: Seq[PublicKey]): Seq[ScriptElt] = {
    ...
    require(
      pubkeys.size > 0 && pubkeys.size <= 16,
