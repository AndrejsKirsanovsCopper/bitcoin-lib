...
object CashAddr {
    ...
    val size = (data.length * 8) match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          "requirement failed: invalid address length"
        )
    }
  ...
  def decodeAddress(address: String): (String, Byte, ByteVector) = {
    ...
    require(
      hrp == "bitcoincash" || hrp == "bchtest" || hrp == "bchreg",
      s"invalid HRP $hrp"
    )
  ...
  def decodeAddressTolerant(address: String): (String, Byte, ByteVector) = {
    if (address.contains(':')) decodeAddress(address)
      ...
      def tryPrefixes(ps: List[String]): (String, Byte, ByteVector) = ps match {
        case Nil =>
          throw new IllegalArgumentException(
            "unable to auto-detect address prefix"
          )
        case prefix :: tail =>
          Try(decodeAddress(s"$prefix:$address")).toOption
            .fold(tryPrefixes(tail))(identity)
      }
