...
object Crypto {
  ...
  private lazy val nativeSecp256k1: Option[Secp256k1] =
    try {
      ...
      case t: Throwable =>
        logger.info(
          "couldn't find secp256k1 library, defaulting to bouncycastle"
        )
        None
    }
  ...
  case class PrivateKey(value: ByteVector32) {
    def add(that: PrivateKey): PrivateKey = nativeSecp256k1.map { h =>
      PrivateKey(
        ByteVector.view(h.privKeyTweakAdd(value.toArray, that.value.toArray))
      )
    } getOrElse {
    ...
    def subtract(that: PrivateKey): PrivateKey = nativeSecp256k1.map { h =>
      PrivateKey(
        ByteVector.view(
          h.privKeyTweakAdd(value.toArray, h.privKeyNegate(that.value.toArray))
        )
      )
    } getOrElse {
    ...
    def multiply(that: PrivateKey): PrivateKey = nativeSecp256k1.map { h =>
      PrivateKey(
        ByteVector.view(h.privKeyTweakMul(value.toArray, that.value.toArray))
      )
    } getOrElse {
    ...
    def publicKey: PublicKey = nativeSecp256k1.map { h =>
      PublicKey.fromBin(ByteVector.view(h.pubkeyCreate(value.toArray)))
    } getOrElse {
      PublicKey(
        ByteVector.view(params.getG().multiply(bigInt).getEncoded(true))
      )
    }
  ...
  object PrivateKey {
    ...
    def fromBase58(value: String, prefix: Byte): (PrivateKey, Boolean) = {
      require(
        Set(Base58.Prefix.SecretKey, Base58.Prefix.SecretKeyTestnet)
          .contains(prefix),
  ...
  case class PublicKey(value: ByteVector) {
    ...
    def multiply(that: PrivateKey): PublicKey = nativeSecp256k1.map { h =>
      PublicKey.fromBin(
        ByteVector.view(h.pubKeyTweakMul(value.toArray, that.value.toArray))
      )
    } getOrElse {
  ...
  object PublicKey {
    def apply(data: ECPoint): PublicKey = new PublicKey(
      ByteVector.view(data.getEncoded(true))
    )
  ...
  def ecdh(priv: PrivateKey, pub: PublicKey): ByteVector32 = {
    nativeSecp256k1.map { h =>
      ByteVector32(
        ByteVector.view(h.ecdh(priv.value.toArray, pub.value.toArray))
      )
    } getOrElse {
      Crypto.sha256(
        ByteVector.view(pub.multiply(priv).ecpoint.getEncoded(true))
      )
    }
  }
  ...
  private def encodeSignatureCompact(t: (BigInteger, BigInteger)): ByteVector =
    encodeSignatureCompact(t._1, t._2)
  ...
  def isValidDERSignatureEncoding(sig: ByteVector): Boolean = {
    ...
    if (lenS > 1 && (sig(startS + 2) == 0x00) && (sig(startS + 3) & 0x80) == 0)
      return false
  ...
  def checkTransactionSignatureEncodingImpl(
      sig: ByteVector,
      flags: Int,
      verifyFunction: ByteVector => Boolean
  ): Boolean = {
  ...
  def checkTransactionSignatureEncoding(
      sig: ByteVector,
      flags: Int
  ): Boolean = {
    checkTransactionSignatureEncodingImpl(
      sig,
      flags,
      checkRawSignatureEncoding(_, flags)
    )
  }

  def checkTransactionECDSASignatureEncoding(
      sig: ByteVector,
      flags: Int
  ): Boolean = {
    checkTransactionSignatureEncodingImpl(
      sig,
      flags,
      checkRawECDSASignatureEncoding(_, flags)
    )
  }

  def checkTransactionSchnorrSignatureEncoding(
      sig: ByteVector,
      flags: Int
  ): Boolean = {
    checkTransactionSignatureEncodingImpl(
      sig,
      flags,
      checkRawSchnorrSignatureEncoding(_, flags)
    )
  }
  ...
  def checkRawECDSASignatureEncoding(sig: ByteVector, flags: Int): Boolean = {
    ...
    if (isSchnorrSig(sig)) {
      ...
      (flags & (SCRIPT_VERIFY_DERSIG | SCRIPT_VERIFY_LOW_S | SCRIPT_VERIFY_STRICTENC)) != 0 && !isValidDERSignatureEncoding(
        sig
      )
    ) false
  ...
  def checkPubKeyEncoding(key: ByteVector, flags: Int): Boolean = {
    ...
    if ((flags & ScriptFlags.SCRIPT_VERIFY_COMPRESSED_PUBKEYTYPE) != 0)
      require(isPubKeyCompressed(key), "public key must be compressed")
    true
  }
  ...
  def getHashType(sig: ByteVector): Int =
    if (sig.isEmpty) 0 else sig.last & 0xff
  ...
  def verifyECDSASignature(
    ...
    nativeSecp256k1.map { h =>
      ...
      h.verify(signature1.toArray, data.toArray, publicKey.value.toArray)
    } getOrElse {
  ...
  def verifySchnorrSignature(
    ...
    require(
      new BigInteger(1, rx.toArray).compareTo(
        curve.getCurve.getField.getCharacteristic
      ) < 0,
      "r must be < P"
    )
    require(
      new BigInteger(1, s.toArray).compareTo(curve.getN) < 0,
      "s must be < N"
    )
    // Compute e = int(hash(bytes(r) || bytes(P) || m)) mod n
    val e = new BigInteger(1, sha256(rx ++ publicKey.value ++ data).toArray)
      .mod(curve.getN)
    require(e.compareTo(one) >= 0, "e must be >= 1")
    // Let R = sG - eP.
    val R =
      PrivateKey(s).publicKey.ecpoint subtract publicKey.ecpoint.multiply(e)
    ...
    val jacobiY = R.getCurve.getCoordinateSystem match {
      case ECCurve.COORD_JACOBIAN | ECCurve.COORD_JACOBIAN_MODIFIED =>
        jacobi(
          R.getYCoord.toBigInteger.multiply(R.getZCoord(0).toBigInteger).mod(p)
        )
      case _ =>
    ...
    val rxMatches = R.getCurve.getCoordinateSystem match {
      case ECCurve.COORD_JACOBIAN | ECCurve.COORD_JACOBIAN_MODIFIED =>
        R.getXCoord.toBigInteger.compareTo(
          new BigInteger(1, rx.toArray)
            .multiply(R.getZCoord(0).toBigInteger.pow(2))
            .mod(p)
        ) == 0
      case _ =>
        R.normalize()
          .getXCoord
          .toBigInteger
          .compareTo(new BigInteger(1, rx.toArray)) == 0
    }
    // Fail if infinite(R) or jacobi(y(R)) ≠ 1 or x(R) ≠ r.
    if (R.isInfinity || jacobiY.compareTo(BigInteger.ONE) != 0 || !rxMatches)
      false
    else true
  }.toOption.getOrElse(false)
  ...
  def sign(data: Array[Byte], privateKey: PrivateKey): ByteVector64 = {
    nativeSecp256k1.map { h =>
      ...
      val privateKeyParameters =
        new ECPrivateKeyParameters(privateKey.bigInt, curve)
