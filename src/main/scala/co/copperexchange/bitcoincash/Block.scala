...
object BlockHeader extends BtcSerializer[BlockHeader] {
  ...
  def blockProof(bits: Long): Double = {
    ...
    if (target == BigInteger.ZERO || negative || overflow) 0.0
      ...
      val work =
        BigInteger.valueOf(2).pow(256).divide(target.add(BigInteger.ONE))
      work.doubleValue()
    }
  }
  ...
  def calculateNextWorkRequired(
    ...
    val powLimit = new BigInteger(
      1,
      hex"00000000ffffffffffffffffffffffffffffffffffffffffffffffffffffffff".toArray
    )
...
case class BlockHeader(
    ...
    bits: Long,
    nonce: Long
) extends BtcSerializable[BlockHeader] {
...
object Block extends BtcSerializer[Block] {
  ...
  override def validate(input: Block): Unit = {
    ...
    require(
      input.tx.map(_.txid).toSet.size == input.tx.size,
      "invalid block: duplicate transactions"
    )
